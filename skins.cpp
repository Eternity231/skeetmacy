...
int manage_skins( int weapon_id ) {
	switch ( weapon_id ) {
		case 0: { } break; // no skins
		case 1: {
			return 3;
		} break;
		case 2: { //westland
...
void Skins::think( ) {
	...
	if( !settings.enable_skinchanger )
		return;
	...
	// store knife index.
	KnifeData* knife = &m_knife_data[ settings.knife_changer ];
	...
	for( int i{ 1 }; i <= g_csgo.m_entlist->GetHighestEntityIndex( ); ++i ) {
		...
		// run knifechanger.
		if( settings.knife_changer > 0 && ent->is( HASH( "CPredictedViewModel" ) ) ) {
			...
			if( weapon->IsKnife( ) ) {
				...
				else if( settings.knife_changer == 0 ) {
					// fix definition.
			...
			switch ( weapon->m_iItemDefinitionIndex( ) ) {
				case GLOCK: {
					weapon->m_nFallbackPaintKit( ) = manage_skins(settings.skin_glock);
					weapon->m_nFallbackStatTrak( ) = true; // because we want all time startrack weapons
					weapon->m_nFallbackSeed( ) = settings.weapon_seed;
					weapon->m_flFallbackWear( ) = ( ( 100.f - settings.weapon_wear ) / 100.f ) + FLT_EPSILON;
				} break;
				case DEAGLE: {
					weapon->m_nFallbackPaintKit( ) = manage_skins( settings.skin_deagle );
					weapon->m_nFallbackStatTrak( ) = true; // because we want all time startrack weapons
					weapon->m_nFallbackSeed( ) = settings.weapon_seed;
					weapon->m_flFallbackWear( ) = ( ( 100.f - settings.weapon_wear ) / 100.f ) + FLT_EPSILON;
				} break;
				case ELITE: {
					weapon->m_nFallbackPaintKit( ) = manage_skins( settings.skin_elite );
					weapon->m_nFallbackStatTrak( ) = true; // because we want all time startrack weapons
					weapon->m_nFallbackSeed( ) = settings.weapon_seed;
					weapon->m_flFallbackWear( ) = ( ( 100.f - settings.weapon_wear ) / 100.f ) + FLT_EPSILON;
				} break;
				case AWP: {
					weapon->m_nFallbackPaintKit( ) = manage_skins( settings.skin_awp );
					weapon->m_nFallbackStatTrak( ) = true; // because we want all time startrack weapons
					weapon->m_nFallbackSeed( ) = settings.weapon_seed;
					weapon->m_flFallbackWear( ) = ( ( 100.f - settings.weapon_wear ) / 100.f ) + FLT_EPSILON;
				...
				case G3SG1: {
					weapon->m_nFallbackPaintKit( ) = manage_skins( settings.skin_auto_t );
					weapon->m_nFallbackStatTrak( ) = true; // because we want all time startrack weapons
					weapon->m_nFallbackSeed( ) = settings.weapon_seed;
					weapon->m_flFallbackWear( ) = ( ( 100.f - settings.weapon_wear ) / 100.f ) + FLT_EPSILON;
				...
				case SCAR20: {
					weapon->m_nFallbackPaintKit( ) = manage_skins( settings.skin_auto_ct );
					weapon->m_nFallbackStatTrak( ) = true; // because we want all time startrack weapons
					weapon->m_nFallbackSeed( ) = settings.weapon_seed;
					weapon->m_flFallbackWear( ) = ( ( 100.f - settings.weapon_wear ) / 100.f ) + FLT_EPSILON;
				...
				case SSG08: {
					weapon->m_nFallbackPaintKit( ) = manage_skins( settings.skin_ssg );
					weapon->m_nFallbackStatTrak( ) = true; // because we want all time startrack weapons
					weapon->m_nFallbackSeed( ) = settings.weapon_seed;
					weapon->m_flFallbackWear( ) = ( ( 100.f - settings.weapon_wear ) / 100.f ) + FLT_EPSILON;
				...
				case USPS: {
					weapon->m_nFallbackPaintKit( ) = manage_skins( settings.skin_usp );
					weapon->m_nFallbackStatTrak( ) = true; // because we want all time startrack weapons
					weapon->m_nFallbackSeed( ) = settings.weapon_seed;
					weapon->m_flFallbackWear( ) = ( ( 100.f - settings.weapon_wear ) / 100.f ) + FLT_EPSILON;
				...
				case REVOLVER: {
					weapon->m_nFallbackPaintKit( ) = manage_skins( settings.skin_revolver );
					weapon->m_nFallbackStatTrak( ) = true; // because we want all time startrack weapons
					weapon->m_nFallbackSeed( ) = settings.weapon_seed;
					weapon->m_flFallbackWear( ) = ( ( 100.f - settings.weapon_wear ) / 100.f ) + FLT_EPSILON;
				...
				case TEC9: {
					weapon->m_nFallbackPaintKit( ) = manage_skins( settings.skin_tec );
					weapon->m_nFallbackStatTrak( ) = true; // because we want all time startrack weapons
					weapon->m_nFallbackSeed( ) = settings.weapon_seed;
					weapon->m_flFallbackWear( ) = ( ( 100.f - settings.weapon_wear ) / 100.f ) + FLT_EPSILON;
...
void Skins::UpdateAnimations( Entity* ent ) {
	int knife = settings.knife_changer;

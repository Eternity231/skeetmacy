	...
	switch ( settings.pene_type ) {
		...
		case 1: { // fatality
			...
			if ( player->m_ArmorValue( ) > 0 ) {
				if ( hitgroup == 1 ) {
					float ratio = g_cl.m_weapon_info->m_armor_ratio;
					float heavy_ratio{};
					float bonus_ratio{};

					if ( player->m_bHasHelmet( ) ) {
						if ( player->m_bHasHeavyArmor( ) ) {
							ratio = g_cl.m_weapon_info->m_armor_ratio * 0.5f;
							heavy_ratio = 0.33f;
							bonus_ratio = 0.33f;
						}
						else {
					...
					if ( calc > player->m_ArmorValue( ) )
						new_damage = damage - player->m_ArmorValue( ) / bonus_ratio;

					damage = new_damage;

					if ( settings.penelog ) {
						std::string log;
						log = "damage: " + std::to_string( damage );

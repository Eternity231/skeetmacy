...
void Aimbot::knife( ) {
	...
	for( const auto& t : m_targets ) {
		...
		// we can history aim.
			...
			LagRecord* best = resolve_record::resolve_data::find_ideal_record( t );//g_resolver.FindIdealRecord( t );
			if( !best )
				continue;
			...
			LagRecord* last = resolve_record::resolve_data::find_last_record( t );//g_resolver.FindLastRecord( t );
			if( !last || last == best )
				continue;

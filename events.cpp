...
void events::round_start( IGameEvent* evt ) { 
	...
	for( int i = 1; i <= g_csgo.m_globals->m_max_clients; ++i ) {
		...
		if ( settings.event_mode_type == 1 ) {
			// reset resolver indexes
			resolve_record::resolve_data::reset_indexes( data );
		}
	}
...
void events::player_hurt( IGameEvent* evt ) {
	...
	if ( settings.event_mode_type == 1 && settings.reset_if_hurt ) {
		for ( int i = 1; i <= g_csgo.m_globals->m_max_clients; ++i ) {
			Player* player = g_csgo.m_entlist->GetClientEntity< Player* >( i );
			if ( !player || player->m_bIsLocalPlayer( ) )
				continue;
	...
	Getze::VisualsControll::add_dmg( evt );
...
void events::bullet_impact( IGameEvent* evt ) {
	...
	ImpactInfo_t info;
	info.x = evt->m_keys->FindKey( HASH( "x" ) )->GetFloat( );
	info.y = evt->m_keys->FindKey( HASH( "y" ) )->GetFloat( );
	info.z = evt->m_keys->FindKey( HASH( "z" ) )->GetFloat( );
	info.time = epochh_time( );
...
void events::item_purchase( IGameEvent* evt ) {
	...
	notify_controller::g_notify.push_event( out );
}
...
void events::player_death( IGameEvent* evt ) {
	...
	// update all players.
		/* // that reset always so we dont use it
			if ( settings.event_mode_type == 1 ) {
				for ( int i = 1; i <= g_csgo.m_globals->m_max_clients; ++i ) {
					Player* player = g_csgo.m_entlist->GetClientEntity< Player* >( i );
					if ( !player || player->m_bIsLocalPlayer( ) )
...
void events::bomb_begindefuse( IGameEvent* evt ) {
	...
	if ( kit ) {
		g_visuals.has_defuser = true;
	}
	else {
		g_visuals.has_defuser = false;
	}

	// debuse start
	g_visuals.started_defusing = true;
...
void events::bomb_abortdefuse( IGameEvent* evt ) {
	...
	// debuse start
	g_visuals.started_defusing = false;

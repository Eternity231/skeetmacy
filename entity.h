...
public:
	...
	__forceinline const char* m_szLastPlaceName( ) {
		auto netvar_get = g_netvars.get( HASH( "DT_BasePlayer" ), HASH( "m_szLastPlaceName" ) );
		return ( const char* )( uintptr_t( this ) + netvar_get );
	}

	__forceinline float& m_flDefuseCountDown( ) {
		auto offset = g_netvars.get( HASH( "DT_PlantedC4" ), HASH( "m_flDefuseCountDown" ) );
		return *( float* )( ( DWORD )this + offset );
	}
	__forceinline float get_bomb_defuse_timer( ) {
		if ( !this )
			return 0.f;

		float returnValue = m_flDefuseCountDown( ) - g_csgo.m_globals->m_curtime;
		return ( returnValue < 0 ) ? 0.f : returnValue;
	}
	__forceinline float get_bomb_blow_timer( )
	{
		if ( !this )
			return 0;

		static auto m_flC4Blow = g_netvars.get( HASH( "DT_PlantedC4" ), HASH( "m_flC4Blow" ) );

		float bombTime = *( float* )( ( DWORD )this + m_flC4Blow );
		float returnValue = bombTime - g_csgo.m_globals->m_curtime;
		return ( returnValue < 0 ) ? 0.f : returnValue;
	}
...
public:
	...
	int m_iModelIndex;
	char pad_[ 11 ];
	Player* m_Player;                     // 0x60
	Weapon* m_ActiveWeapon;        // 0x64
	Weapon* m_LastActiveWeapon;	  // 0x68
	...
	vec3_t m_vecNormalizedVelocity;			  // 0xD4 velocity * (1.0 / velocity_length)
	vec3_t m_vecNormalizedMovingVelocity;	  // 0xE0
	...
	vec3_t m_vecLeanVelocity;                       // 0x16D
	vec3_t m_velocityUnk2;                          // 0x17F
	vec3_t m_leanVelocity1;                         // 0x191
	vec3_t m_leanVelocity2;
	char pad11[ 32 ];
	bool m_bUnknown01;
	...
	static void ModifyEyePosition( CCSGOPlayerAnimState* pState, vec3_t* pos );
};//Size=0x344
...
class CBoneAccessor {
public:
	__forceinline int GetReadableBones( ) {
		return m_ReadableBones;
	}

	__forceinline void SetReadableBones( int flags ) {
		m_ReadableBones = flags;
	}

	__forceinline int GetWritableBones( ) {
		return m_WritableBones;
	}

	__forceinline void SetWritableBones( int flags ) {
...
public:
	...
	CUtlVector<C_AnimationLayer>& m_AnimOverlay2( ) {
	...
	__forceinline vec3_t& m_aimPunchAngleVec( ) {
		return get< vec3_t >( g_entoffsets.m_aimPunchAngle );
	}
	...
	__forceinline vec3_t* m_vecBonePos( ) {
		static auto bonecache = pattern::find( g_csgo.m_client_dll, "8D 87 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 8B 06" );
		return ( vec3_t* )( ( uintptr_t )this + *( int* )( bonecache + 0x2 ) + 0x4 );
	}

	__forceinline quaternion_t* m_quatBoneRot( ) {
		static auto bonecache = pattern::find( g_csgo.m_client_dll, "8D 87 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 8B 06" );
		return ( quaternion_t* )( ( uintptr_t )this + *( int* )( bonecache + 0x25 ) + 0x4 );

	}

	__forceinline bool& m_bIsJiggleBonesEnabled( ) {
		return *( bool* )( ( uintptr_t )this + g_entoffsets.m_hLightingOrigin - 0x18 );
	}
...
public:
	...
	__forceinline bool should_fix_eye_pos( ) {
		if ( m_PlayerAnimState( ) == nullptr )
			return false;

		return m_PlayerAnimState( )->m_land || !( m_fFlags( ) & FL_ONGROUND );
	}
	...
	__forceinline ang_t get_eye_pos_ang( )
	{
		ang_t out;
		util::get_method<void( __thiscall* )( void*, ang_t& )>( this, 277 )( this, out );
		return out;
	}
	...
	__forceinline void FixSetupBones( matrix3x4_t* Matrix )
	{
		int Backup = *( int* )( ( uintptr_t )this + 0x274 );
		*( int* )( ( uintptr_t )this + 0x274 ) = 0;
		vec3_t absOriginBackupLocal = this->GetAbsOrigin( );
		this->SetAbsOrigin( this->m_vecOrigin( ) );
		this->SetupBones( Matrix, 128, 0x00000100, g_csgo.m_globals->m_curtime );
...
public:
	...
	__forceinline float FireRate( )
	{
		if ( !this )
			return 0.f;

		std::string WeaponName = GetWpnData( )->m_weapon_name;
	...
	__forceinline int& m_nSmokeEffectTickBegin( ) {
		auto offset = g_netvars.get( HASH( "DT_SmokeGrenadeProjectile" ), HASH( "m_nSmokeEffectTickBegin" ) );

...
namespace ShotInformations {
	struct ShotSnapshot {
		...
		C_CSPlayer* player;
		LagController::C_LagRecord resolve_record;
		Vector eye_pos;
		float time;
		bool doubleTap;
	...
	public:
		...
		void CreateSnapshot( C_CSPlayer* player, const Vector& shootPosition, const Vector& aimPoint, LagController::C_LagRecord* record, int resolverSide, int hitgroup, int hitbox, int nDamage, bool doubleTap = false );

...
ulong_t __stdcall Client::init( void* arg ) {
	...
	// welcome the user.
	notify_controller::g_notify.push_event( tfm::format( XOR( "welcome %s\n" ), g_cl.m_user ) );
...
Color to_color_alpha( int col[ 4 ] ) {
...
void Client::DrawHUD( ) {
	...
	std::string text = tfm::format( XOR( "skeetmacy | rtt: %ims | rate: %i | %s" ), ms, rate, time.str( ).data( ) );
...
void Client::OnPaint( ) {
	...
	g_visuals.think( );
	g_grenades.paint( );
	notify_controller::g_notify.think( );
...
void Client::DoMove( ) {
	...
	if ( !TickbaseControll::m_shifting ) {
		...
		// run aimbot.
		g_aimbot.think( );
		//StackHack::AimbotControll::on_create_move( g_cl.m_cmd, g_cl.m_packet );  doesnt works as is supposed to
	}
	else {
		*m_packet = TickbaseControll::m_tick_to_shift == 1;
...
void Client::OnTick( CUserCmd* cmd ) {
	...
	g_inputpred.restore( );
	if ( !TickbaseControll::m_shifting ) {
		TickbaseControll::manipulate_tickbase( );
	}
}
...
void Client::print( const std::string text, ... ) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf( to_color_alpha( settings.menu_color ), XOR( "[skeetmacy] " ) );
